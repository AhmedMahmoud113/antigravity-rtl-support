...
// CSS for WEBVIEW iframes (chat panel, markdown preview)
// Generic selectors are safe here because webviews only contain chat/md content
const WEBVIEW_CSS = `
/* === RTL Fix — Webview Content === */
...
ul, ol { direction: rtl; }
ol > li, ul > li { text-align: right; }

body, .rendered-markdown, .chat-markdown-part,
.markdown-content, .vscode-body, .markdown-body {
  direction: rtl;
}

/* Code blocks — ALWAYS LTR */
pre, code, .monaco-editor, .interactive-result-code-block {
  direction: ltr !important;
  text-align: left !important;
  unicode-bidi: isolate !important;
}
...
table { direction: ltr; }
td, th { unicode-bidi: plaintext; text-align: start; }
`;
...
// Scoped CSS for main window (only targets chat panel, not menus/tabs/editor)
const MAIN_CSS = ${JSON.stringify(RTL_CSS)};

// Generic CSS for webview iframes (safe — webviews only contain chat/md content)
const WEBVIEW_CSS = ${JSON.stringify(WEBVIEW_CSS)};

app.on('web-contents-created', (_event, webContents) => {
  webContents.on('dom-ready', () => {
    const url = webContents.getURL();
    if (url.startsWith('vscode-webview:')) {
      // Webview iframe — use generic selectors (only chat/md content here)
      webContents.insertCSS(WEBVIEW_CSS).catch(() => {});
    } else {
      // Main window — use scoped selectors (don't affect menus/tabs/editor)
      webContents.insertCSS(MAIN_CSS).catch(() => {});
    }
  });
});
